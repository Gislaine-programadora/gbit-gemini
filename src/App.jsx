...
const apiKey = ""; // API key será preenchida pelo ambiente de execução
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-4 sm:p-8 font-sans">
      ...
      <main className="max-w-7xl mx-auto mt-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            {/* Cartões de Ferramenta Rápida */}
            <div className="space-y-4">
              {Object.entries(toolInfo).map(([key, info]) => (
                 <div key={key} className={`bg-gray-800 p-4 rounded-xl shadow-lg border-l-4 border-${info.color}-500 transition-all hover:shadow-xl`}>
                    <div className="flex items-center justify-between">
                       <h4 className={`text-lg font-semibold text-${info.color}-400 flex items-center`}>
                           <info.icon className="w-5 h-5 mr-2" />
                           {info.title}
                       </h4>
                       <button onClick={() => setActiveTool(key)} className={`text-sm font-medium px-3 py-1 rounded-full bg-gray-700 hover:bg-${info.color}-600 hover:text-white text-gray-300 transition-colors`}>
                           Abrir
                       </button>
                    </div>
          ...
          <div className="bg-gray-800 p-6 rounded-xl shadow-2xl min-h-[600px] flex flex-col h-full border border-purple-700/50">
            ...
            <h3 className="text-2xl font-bold text-white flex items-center mb-4 pb-2 border-b border-gray-700">
              <Icon className={`w-6 h-6 mr-2 text-${color}-400`} />
              {title}
            </h3>

            {/* Seletor de Ferramentas (Tabs) */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 flex-shrink-0 mb-6">
              {Object.entries(toolInfo).map(([key, info]) => (
                <button
                  key={key}
                  onClick={() => setActiveTool(key)}
                  className={`flex items-center justify-center space-x-1 px-2 py-2 text-xs sm:text-sm font-semibold rounded-lg transition duration-200 border-2 border-transparent ${
                    activeTool === key
                      ? `bg-${info.color}-700 text-white shadow-md border-${info.color}-500`
      ...
      {/* Estilos para a barra de rolagem customizada e ajustes de layout */}
      <style jsx global>{`
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb {
          background: #5b21b6; /* purple-700 (Cor padrão, mas adaptável) */
          border-radius: 4px;
        }
        ...
        /* Estilos específicos para classes dinâmicas do Tailwind (necessário para que o JIT as reconheça) */
        .text-purple-400 {} .border-purple-500 {} .bg-purple-700 {} .hover:bg-purple-600 {}
        .text-green-400 {} .border-green-500 {} .bg-green-700 {} .hover:bg-green-600 {}
        .text-orange-400 {} .border-orange-500 {} .bg-orange-700 {} .hover:bg-orange-600 {}
        .text-yellow-400 {} .border-yellow-500 {} .bg-yellow-700 {} .hover:bg-yellow-600 {}
      `}</style>
    </div>
  );
}
